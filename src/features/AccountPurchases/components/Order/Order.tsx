...
export const PurchaseOrder = ({ order }: PropsWithChildren<{ order: Order }>) => {
  return (
    <div className="shadow sm:rounded-md sm:overflow-hidden bg-background p-2 sm:p-4">
      ...
      <header className="bg-mainText-50 rounded-lg p-2 sm:p-4 sm:flex sm:flex-wrap sm:items-center sm:justify-between">
        <dl className="divide-y divide-mainText-200 space-y-6 text-sm text-mainText-600 flex-auto sm:divide-y-0 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-x-6 lg:gap-x-8">
          <div className="flex justify-between sm:block">
            <dt className="font-medium text-mainText-900">Date placed</dt>
            <dd className="sm:mt-1">
              <time dateTime={order.createdAt}>{format(new Date(order.createdAt), 'PP')}</time>
            </dd>
          </div>
          <div className="flex justify-between pt-6 sm:block sm:pt-0">
            <dt className="font-medium text-mainText-900">Order number</dt>
        ...
        <div className="w-full pt-2 border-t-mainText-200"></div>
        {/* <a
          href={order.invoiceHref}
          className="w-full flex items-center justify-center bg-background mt-6 py-2 px-4 border border-mainText-300 rounded-md shadow-sm text-sm font-medium text-mainText-700 hover:bg-mainText-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent-500 sm:w-auto sm:mt-0"
        >
          View Invoice
