    ...
    <Transition.Root show={isSearchOpen} as={Fragment} afterLeave={onLeave} appear>
      <Dialog as="div" className="relative z-10" open={isSearchOpen} onClose={() => setIsSearchOpen(false)}>
        ...
        <div className="fixed inset-0 z-10 overflow-y-auto p-4 sm:p-6 md:p-20">
          ...
          >
            <Dialog.Panel className="mx-auto max-w-xl transform divide-y divide-formText-100 overflow-hidden rounded-xl bg-background shadow-2xl ring-1 ring-black ring-opacity-5 transition-all">
              <Combobox value={query} onChange={onSelectResult}>
                <div className="relative">
                  <SearchIcon
                    className="pointer-events-none absolute top-3.5 left-4 h-5 w-5 text-formText-400"
                    aria-hidden="true"
                  />
                  <Combobox.Input
                    className="h-12 w-full border-0 bg-transparent pl-11 pr-4 text-formText-800 placeholder-formText-400 focus:ring-0 sm:text-sm"
                    placeholder="Search..."
                ...
                {query !== '' && results.length === 0 && !loading && (
                  <div className="py-14 px-6 text-center text-sm sm:px-14">
                    <ExclamationIcon className="mx-auto h-6 w-6 text-mainText-400" aria-hidden="true" />
                    <p className="mt-4 font-semibold text-mainText-900">No results found</p>
                    <p className="mt-2 text-mainText-500">
