...
export const ProductPriceSelect = ({ value, onChange, options }: ProductPriceSelectProps) => {
  return (
    <RadioGroup value={value} onChange={onChange}>
      ...
      <div className="relative bg-formBackground rounded-md -space-y-px">
          ...
          <RadioGroup.Option
            ...
            className={({ checked }) =>
              classNames(
                ...
                checked ? 'bg-accent-50 border-accent-200 z-10' : 'border-formText-200',
            ...
            {({ active, checked }) => (
              <>
                  ...
                  <div>
                    <span
                      className={classNames(
                        checked ? 'bg-accent-600 border-transparent' : 'bg-formBackground border-formText-300',
                        active ? 'ring-2 ring-offset-2 ring-accent-500' : '',
                        'h-4 w-4 rounded-full border flex items-center justify-center'
                      )}
                      aria-hidden="true"
                    >
                      <span className="rounded-full bg-formBackground w-1.5 h-1.5" />
                    </span>
                  </div>
                  <RadioGroup.Label
                    as="span"
                    className={classNames(checked ? 'text-accent-900' : 'text-formText-900', 'ml-3 font-medium')}
                  >
                ...
                <RadioGroup.Description
                  as="span"
                  className={classNames(
                    checked ? 'text-accent-700' : 'text-formText-500',
